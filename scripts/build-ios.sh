...
if [ $1 ]; then
  ...
  if [ $2 ]; then
    ...
    echo "Error: App version is missing ./build-ios.sh [APP_DIRECTORY] [PACKAGE_NAME] [APP_VERSION]"
  fi
else
  echo "Error: Package name is missing ./build-ios.sh [[APP_DIRECTORY] [PACKAGE_NAME] [APP_VERSION]"
fi
else
  echo "Error: Application directory is missing ../build-ios.sh [APP_DIRECTORY] [PACKAGE_NAME] [APP_VERSION]"
fi